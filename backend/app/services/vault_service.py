...
def _render_record_pdf(
    ...
    *,
    ...
    heading_style = ParagraphStyle(
        "Section", parent=styles["Heading2"], fontSize=10, spaceBefore=8, spaceAfter=2,
        textColor=HexColor(accent),
    )
    body_style = ParagraphStyle("Body", parent=styles["BodyText"], fontSize=10, leading=14)
    meta_label_style = ParagraphStyle(
        "MetaLabel", parent=styles["BodyText"], fontSize=9.5, leading=13,
        textColor=HexColor("#64748b"),
    )
    meta_value_style = ParagraphStyle(
        "MetaValue", parent=styles["BodyText"], fontSize=10, leading=13, fontName="Helvetica-Bold",
    )
    footer_style = ParagraphStyle(
        "Footer", parent=styles["BodyText"], fontSize=7.5, leading=10, textColor=HexColor("#94a3b8"),
    )
        ...
        table.setStyle(
            TableStyle(
                [
                    ...
                    ("VALIGN", (0, 0), (-1, -1), "TOP"),
                    # A divide-y row list (not a boxed grid) so a long meta
                    # table stays readable without looking like a spreadsheet.
    ...
    for label, text_value in sections:
        ...
        story.append(Paragraph(_escape_pdf_text(label).upper(), heading_style))
        story.append(HRFlowable(width="100%", thickness=1, color=HexColor(accent), spaceAfter=3))
        for line in text_value.splitlines() or [text_value]:
            story.append(Paragraph(_escape_pdf_text(line) or " ", body_style))

    story.append(Spacer(1, 6 * mm))
    story.append(HRFlowable(width="100%", thickness=0.5, color=HexColor("#e2e8f0"), spaceAfter=3))
    story.append(Paragraph(
        f"Generated by CareCliQ &middot; {datetime.now(timezone.utc).strftime('%d %b %Y, %H:%M UTC')}",
        footer_style,
    ))
