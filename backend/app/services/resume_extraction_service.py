"""Extract a short candidate profile (summary, skills, experience, and any
credentials mentioned) from an uploaded resume/CV, so the Applicants Board
doesn't just store the file — it builds a real profile that carries onto
the worker's record on hire and can be reused later for rostering/
participant skill-matching.

Credentials extraction is deliberately a summary only, normalized against
the same fixed vocabulary the real Credentials system uses (see
CREDENTIAL_TYPES below) — it tells a hiring manager what to expect and
follow up on, not a verified record. Nothing here writes to
public.credentials; the candidate/worker still has to upload the actual
...
# Same fixed vocabulary the real Credentials system uses (FIXED_CREDENTIAL_TYPES
# in backend/app/api/compliance.py) — a resume-mentioned "First Aid Certificate"
# is only useful to a hiring manager if it's normalized to the same type name
# the actual credential record will use once the real document is uploaded.
CREDENTIAL_TYPES = [
...
from the document — use null (or an empty array for skills/credentials) for anything not clearly \
present. Never guess or invent a value, and never include the candidate's name, contact details, \
or anything not related to their work profile.

Also look for any mention of these specific credentials (certificates, checks, or training the \
candidate says they hold): NDIS Worker Screening Check, Working With Children Check (WWCC), NDIS \
Code of Conduct training, First Aid certificate, CPR certificate, Manual Handling training, \
Infection Control training, Medication Administration training/certificate. Only include one if \
the resume actually mentions it (by that name or a clear equivalent) — this is a self-reported \
summary for a hiring manager to follow up on, not a verified record, so do not infer one exists \
just because the role would typically require it.
...
Respond with exactly this JSON shape:
{
  ...
  "credentials": [{"type": "<one of: ndis_screening, wwcc, code_of_conduct, first_aid, cpr, manual_handling, infection_control, medication_admin>", "mentioned_as": "<the exact or near-exact phrase from the resume, e.g. 'First Aid Certificate (2024)'>"}]
}"""
...
def _sanitize(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(credentials_raw, list):
        ...
        for entry in credentials_raw:
            ...
            if ctype not in CREDENTIAL_TYPES or ctype in seen_types:
                continue  # ignore anything outside the fixed vocabulary or a duplicate
            mentioned_as = str(entry.get("mentioned_as") or "").strip()
            credentials.append({"type": ctype, "mentioned_as": mentioned_as or ctype})
    ...
    return {
        ...
        "credentials_claimed": credentials or None,
    }
...
async def extract_and_save_applicant_profile(
    ...
    if (
        not fields["resume_summary"]
        and not fields["resume_skills"]
        and not fields["resume_experience_years"]
        and not fields["credentials_claimed"]
    ):
        return None
