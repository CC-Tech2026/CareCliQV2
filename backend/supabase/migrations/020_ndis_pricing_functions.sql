-- ─────────────────────────────────────────────────────────────────────────────
-- NDIS Pricing Functions and Tables
-- ─────────────────────────────────────────────────────────────────────────────

-- Create NDIS price schedules table
...
ALTER TABLE ndis_price_schedules ENABLE ROW LEVEL SECURITY;
ALTER TABLE ndis_price_items ENABLE ROW LEVEL SECURITY;

-- RLS policies for price schedules
CREATE POLICY "Users can view price schedules for their org" ON ndis_price_schedules
    FOR SELECT USING (
        organization_id = (SELECT organization_id FROM users WHERE id = auth.uid())
    );

CREATE POLICY "Coordinators can insert price schedules" ON ndis_price_schedules
...
CREATE POLICY "Users can view price items for their org" ON ndis_price_items
    FOR SELECT USING (
        organization_id = (SELECT organization_id FROM users WHERE id = auth.uid())
    );

CREATE POLICY "Coordinators can insert price items" ON ndis_price_items
    FOR INSERT WITH CHECK (
        organization_id = (SELECT organization_id FROM users WHERE id = auth.uid()) AND
        EXISTS (
            SELECT 1 FROM organization_members
            WHERE user_id = auth.uid() AND
                  organization_id = ndis_price_items.organization_id AND
                  role = 'support_coordinator'
        )
    );

-- ─────────────────────────────────────────────────────────────────────────────
-- resolve_ndis_price function
-- Resolves effective price for an NDIS item at a point in time
-- ─────────────────────────────────────────────────────────────────────────────
