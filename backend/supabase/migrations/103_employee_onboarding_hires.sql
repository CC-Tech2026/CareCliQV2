...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'employee_onboarding' AND policyname = 'employee_onboarding_service_role') THEN
        CREATE POLICY employee_onboarding_service_role ON public.employee_onboarding FOR ALL TO service_role USING (true) WITH CHECK (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'employee_onboarding_documents' AND policyname = 'employee_onboarding_documents_service_role') THEN
        CREATE POLICY employee_onboarding_documents_service_role ON public.employee_onboarding_documents FOR ALL TO service_role USING (true) WITH CHECK (true);
    END IF;
END $$;
