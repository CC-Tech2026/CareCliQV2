...
-- Folded in from backend/supabase/migrations/101_chatbox_messages.sql.
-- Replaces the in-process MemorySaver, which lost all history on every
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'chatbox_messages' AND policyname = 'owner_all'
    ) THEN
