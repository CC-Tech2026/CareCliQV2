...
function defaultRouteForSource(source: string | undefined): string {
  if (source === "credentials" || source === "worker-compliance")
    return "/md/staff";
  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 max-w-2xl flex-1 items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
            ...
            {alert.due_date && (
              ...
              >
                Due{" "}
                {new Date(alert.due_date).toLocaleDateString("en-AU", {
                  day: "numeric",
                  month: "short",
                })}
              </span>
        ...
        <span
          className="mt-0.5 shrink-0 whitespace-nowrap text-[10.5px] font-black"
          style={{ color }}
        >
          {alert.action_label}
        </span>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 max-w-2xl flex-1 space-y-2">
        ...
        />
        <div
          className="h-2 w-4/5 animate-pulse rounded"
          style={{ background: "var(--cc-soft)" }}
        />
      </div>
    </div>
...
function EmptyState({ message }: { message: string }) {
  return (
    <div className="flex flex-col items-center justify-center px-5 py-8 text-center">
      ...
      <p className="mt-3 text-[11.5px] font-bold" style={{ color: TEXT }}>
        {message}
      </p>
    </div>
  );
...
export function GovernanceTriage({
  ...
  }, [fetchAlerts, retryKey]);

  if (error)
    return (
      <div
        role="alert"
        className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-cc-border bg-cc-surface p-4"
      >
        <div className="min-w-0">
        ...
        <button
          ...
          className="min-h-11 rounded-lg border border-cc-border px-4 text-sm font-medium text-cc-plum hover:bg-cc-soft"
        >
          Retry action items
        </button>
      </div>
  ...
  const topSeverity = all.some((a) => a.severity === "critical")
    ? "critical"
    : all.some((a) => a.severity === "high")
      ? "high"
      : all.length > 0
        ? "medium"
        : null;
  const topSeverityColors = topSeverity
    ? severityColors(topSeverity)
      ...
      <div className="relative">
        ...
        >
          <HeaderIcon
            size={15}
            strokeWidth={2}
            style={{ color: topSeverityColors.color }}
          />
          Attention
          {!loading && all.length > 0 && (
            <span
              className="flex h-4 min-w-[17px] items-center justify-center rounded-full px-1 text-[9.5px] font-black text-white"
              style={{ background: topSeverityColors.color }}
            >
              {all.length}
            </span>
          )}
            ...
            >
              ...
              >
                ...
                <button
                  onClick={() => setFloatingOpen(false)}
                  aria-label="Close"
                  className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full hover:bg-cc-soft"
                >
  ...
  if (variant === "sidebar") {
    return (
      ...
      >
        <div
          className="flex items-center justify-between gap-2 border-b px-5 py-4"
          style={{ borderColor: BORDER }}
        >
          <h2 className="text-[13px] font-black" style={{ color: TEXT }}>
            Needs your attention
          </h2>
          {!loading && all.length > 0 && (
            <span
    ...
    >
      <div
        ...
        style={{
          borderColor: BORDER,
          borderBottom:
            !collapsed && all.length > 0 ? `1px solid ${BORDER}` : "none",
        }}
      >
        <button
        ...
        >
          <div
            ...
            style={{ background: topSeverityColors.soft }}
          >
            <HeaderIcon
              size={16}
              strokeWidth={2}
        ...
        <div className="flex items-center gap-2.5">
          {!loading && all.length > 0 && topSeverity && (
            <span
              className="rounded-full px-2.5 py-1 text-[9.5px] font-black uppercase tracking-wide"
              style={{
                background: topSeverityColors.soft,
                color: topSeverityColors.color,
              }}
            >
