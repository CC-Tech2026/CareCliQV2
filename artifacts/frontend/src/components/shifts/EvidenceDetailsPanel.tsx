...
export function EvidenceDetailsPanel({ metadata, defaultOpen = false, showRetention = true }: Props) {
  ...
  return (
    <div className="rounded-xl border border-[#E2DEF2] bg-[#FAFAFE]">
      <button
        type="button"
      ...
      {open && (
        <dl className="space-y-2 border-t border-[#E2DEF2] px-3 py-3 text-xs">
          ...
          {showRetention && metadata.retention_until && (
            <p className="rounded-lg border border-[#E2DEF2] bg-white px-2.5 py-2 text-xs font-semibold leading-snug" style={{ color: TEXT }}>
