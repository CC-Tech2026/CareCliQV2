...
const GOAL_STATUS_CONFIG = {
  not_started: { label: "Not Started", cls: "bg-cc-bg text-cc-muted border-cc-border", icon: Circle },
  in_progress: { label: "In Progress", cls: "bg-[#FFE8EE] text-cc-coral border-[#F8C0CE]", icon: Activity },
  ...
  if (msg.type === "system") {
    return (
      <div className="flex justify-center my-2 px-4">
        <span className="rounded-full bg-cc-surface/80 px-3 py-1 text-[10px] font-medium text-cc-muted shadow-sm">{msg.content}</span>
      </div>
    );
  }
  ...
  if (msg.type === "activity") {
    ...
    return (
      <div className="flex justify-center my-1.5">
        <div className="flex items-center gap-1.5 bg-cc-surface border border-cc-border rounded-full px-3 py-1 shadow-sm">
          <AIcon className="h-2.5 w-2.5 text-cc-plum shrink-0" />
          <span className="text-[10px] text-cc-text font-semibold">{msg.activityType || msg.content}</span>
          <span className="text-[9px] text-cc-muted">• {format(msg.timestamp, "HH:mm")}</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex justify-center my-1.5">
        <div className="flex items-center gap-1.5 bg-[#FFE8EE] border border-[#F8C0CE] rounded-full px-3 py-1 shadow-sm">
          <Target className="h-2.5 w-2.5 text-cc-coral shrink-0" />
          <span className="text-[10px] text-cc-text font-semibold">{msg.content}</span>
          <span className="text-[9px] text-cc-muted">• {format(msg.timestamp, "HH:mm")}</span>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex justify-end px-1 my-0.5">
      <div className="max-w-[82%] min-w-[60px]">
        <div className="bg-cc-surface border border-cc-border rounded-2xl rounded-tr-sm overflow-hidden shadow-sm">
          ...
          {msg.type === "file" && (
            <div className="px-4 py-3 flex items-center gap-3">
              <div className="h-9 w-9 bg-cc-bg rounded-lg flex items-center justify-center shrink-0">
                <FileText className="h-4 w-4 text-cc-plum" />
              </div>
              <div className="min-w-0">
                <p className="text-cc-text text-sm font-semibold leading-tight truncate">{msg.content}</p>
                <p className="text-cc-muted text-[10px]">Document attached</p>
              </div>
            </div>
          )}
            ...
            <div className="px-4 py-3">
              {msg.type === "voice" && (
                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                  <div className="flex items-center gap-1 text-cc-plum">
                  ...
                  {msg.detectedLanguage && msg.detectedLanguage !== "en" && (
                    <span className="text-[9px] bg-cc-bg text-cc-muted px-1.5 py-0.5 rounded-full">
                      {msg.detectedLanguage.toUpperCase()}
                    </span>
                  )}
                  {msg.isTranslating && (
                    <div className="flex items-center gap-1 text-cc-muted">
              ...
              {(msg.type === "text" || msg.type === "voice") && (
                <p className="text-cc-text text-sm leading-relaxed">{msg.content}</p>
              )}
              {msg.type === "voice" && (
                <div className="mt-2 pt-2 border-t border-[#E8D5E8]">
                  <div className="mb-1">
                    {msg.isTranslating ? (
                      <span className="text-[9px] text-cc-muted">Translating...</span>
                    ) : msg.translationStatus === "translated" ? (
                      <span className="text-[9px] font-bold uppercase tracking-wider text-cc-plum">Translated to English</span>
                      ...
                      <span className="text-[9px] text-cc-muted">English legal output</span>
                    ) : null}
                  </div>
                  {msg.translated ? (
                    <>
                    ...
                    <p className="text-cc-muted text-xs italic">English translation unavailable</p>
                  ) : null}
                </div>
              )}
            </div>
...
export default function SessionLive() {
  ...
  if (isLoading) {
    return (
      <div className="h-[calc(100vh-9rem)] min-h-[520px] flex items-center justify-center rounded-[2rem] bg-cc-surface border border-cc-border">
        <Loader2 className="h-8 w-8 animate-spin text-cc-plum" />
      </div>
    );
  }
  ...
  if (!session) {
    return (
      <div className="h-[calc(100vh-9rem)] min-h-[520px] flex flex-col items-center justify-center gap-4 rounded-[2rem] bg-cc-surface border border-cc-border">
        <p className="text-cc-muted">Session not found</p>
  ...
  return (
    <div className="relative mx-auto flex h-[calc(100vh-9rem)] min-h-[620px] max-h-[780px] max-w-5xl flex-col overflow-hidden rounded-[2rem] border border-cc-border bg-cc-surface shadow-[0_14px_40px_rgba(84,34,105,0.08)]">

      {/* ── Top control bar ── */}
      <div
        className="shrink-0 border-b border-[#E8D5E8] bg-cc-surface"
      >
        <div className="flex items-center justify-between gap-3 px-4 py-3">
          <button
            onClick={() => navigate(`/sessions/${id}`)}
            className="flex items-center gap-1.5 text-cc-muted hover:text-cc-plum text-sm transition-colors font-semibold"
          >
          ...
          <div className="min-w-0 flex-1 px-1">
            ...
            <p className="text-cc-muted text-[11px] capitalize">{session.session_type?.replace(/_/g, " ")}</p>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <select
              ...
              className="h-9 max-w-[118px] sm:max-w-[160px] rounded-full border border-cc-border bg-cc-bg px-3 text-[12px] font-bold text-cc-text outline-none hover:bg-cc-bg disabled:opacity-50"
            >
            ...
            <div className="hidden sm:flex items-center gap-1">
              <Globe className="h-3.5 w-3.5 text-cc-muted shrink-0" />
              <div className="flex rounded-full border border-cc-border bg-cc-bg p-0.5 overflow-hidden">
                {(["original", "translated", "both"] as TranslationView[]).map((v) => (
                  <button
                    ...
                    className={cn(
                      ...
                      translationView === v
                        ? "bg-cc-surface text-cc-plum shadow-sm"
                        : "text-cc-muted hover:text-cc-plum",
                    )}
                  >
            ...
            <button
              ...
              className="h-9 w-9 rounded-full border border-cc-border bg-cc-surface text-cc-plum hover:bg-cc-bg disabled:opacity-40 flex items-center justify-center transition-colors"
            >
            ...
            {elapsed > 0 && (
              <button
                ...
                className="hidden sm:inline-flex text-cc-muted hover:text-cc-plum text-[11px] font-semibold px-2 py-1 rounded-lg hover:bg-cc-bg transition-colors"
              >
                Restart
              </button>
            )}
        ...
        <div className="flex items-center justify-between gap-3 px-4 py-2 border-t border-[#F0ECFA] bg-[#F8F6FF]">
          <div className="flex items-center gap-2.5">
            <span className="font-mono text-lg font-black text-cc-text tracking-tight">
            ...
            <span
              className={cn(
                ...
                isActive
                  ...
                  : "bg-cc-surface text-cc-muted border-cc-border",
              )}
            >
              <span
                className={cn(
          ...
          <div className="flex items-center gap-3 text-[10px] font-medium text-cc-muted">
      ...
      {goals.length > 0 ? (
        <div className="shrink-0 border-b border-[#E8D5E8] bg-cc-surface px-3 py-2">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider text-cc-muted shrink-0">
            ...
            <button
              ...
              className="ml-auto flex items-center gap-1 text-[10px] font-semibold text-cc-muted hover:text-cc-plum transition-colors"
            >
      ...
      {bodyMapOpen && (
        <div className="shrink-0 bg-[#F8F6FF] border-b border-[#E8D5E8] px-4 py-4 max-h-[260px] overflow-y-auto">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[10px] font-bold text-cc-muted uppercase tracking-wider flex items-center gap-2">
              <HeartPulse className="h-3.5 w-3.5" style={{ color: "#F1738A" }} /> Physical Examination
            </h3>
            <button
              onClick={() => setBodyMapOpen(false)}
              className="text-cc-muted hover:text-cc-plum"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto bg-[#F6F4FB] px-3 py-4">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center py-16">
            ...
            <p className="text-cc-text text-sm font-bold">Session started</p>
            <p className="text-cc-muted text-xs mt-1">Document notes, voice, and evidence below.</p>
          </div>
        )}
        ...
        {isRecording && (
          <div className="flex justify-end px-1 mt-1">
            <div className="max-w-[82%] bg-cc-surface border border-[#F8C0CE] rounded-2xl rounded-tr-sm px-4 py-3 shadow-sm">
              <div className="flex items-center gap-2 mb-1.5 text-cc-coral">
                <Radio className="h-3 w-3 animate-pulse" />
                <span className="text-[9px] font-bold uppercase tracking-wider">Listening…</span>
              </div>
              <p className="text-cc-muted text-sm italic">
      ...
      {/* ── Bottom input bar ── */}
      <div className="shrink-0 bg-cc-surface px-3 sm:px-4 py-3 border-t border-cc-border shadow-[0_-8px_24px_rgba(84,34,105,0.06)]">
        <div className="client-translation-composer flex items-center gap-2 sm:gap-3">
          <div className="message-pill min-w-0 flex-1 h-12 sm:h-[52px] rounded-full bg-cc-surface border border-cc-border shadow-sm flex items-center pl-4 sm:pl-5 pr-1.5">
            <input
              value={inputText}
              ...
              placeholder="Message"
              disabled={!isActive}
              className="min-w-0 flex-1 bg-transparent text-cc-text text-[15px] placeholder:text-[#9A8BC4] outline-none disabled:opacity-50"
            />
            <button
              type="button"
              ...
              disabled={!isActive || isUploadingAttachment}
              className="h-10 w-10 rounded-full flex items-center justify-center text-cc-plum hover:bg-cc-bg transition-colors disabled:opacity-40 shrink-0"
            >
          ...
          <button
            ...
            className={cn(
              "voice-circle h-12 w-12 sm:h-14 sm:w-14 rounded-full flex items-center justify-center text-white shadow-[0_8px_20px_rgba(85,51,204,0.24)] ring-4 ring-white border border-cc-border transition-all shrink-0 disabled:opacity-45",
              isRecording
      ...
      {showActivitySheet && (
        ...
        >
          <div
            className="w-full bg-cc-surface border-t border-cc-border rounded-t-3xl shadow-2xl px-4 pt-4 pb-8 animate-in slide-in-from-bottom-4 duration-200"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-cc-text font-bold text-sm">Log Activity</h3>
              <button
                onClick={() => setShowActivitySheet(false)}
                className="text-cc-muted hover:text-cc-plum"
              >
                ...
                return (
                  <div key={cat.label}>
                    <div className="flex items-center gap-1.5 mb-2">
                      <CatIcon className="h-3 w-3 text-cc-muted" />
                      <span className="text-[9px] font-bold uppercase tracking-wider text-cc-muted">
                        {cat.label}
                      </span>
                    </div>
      ...
      >
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl shadow-2xl p-0 bg-cc-surface" style={{ border: "1px solid rgba(232,213,232,0.5)" }}>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <SmartTextarea
                  ...
                  style={{ background: 'var(--cc-surface)', borderColor: "rgba(232,213,232,0.5)", color: "#4A3D5A" }}
                />
              </div>
                  ...
                  {previewProgressLoading ? (
                    ...
                    <ul className="space-y-2">
                      {previewDeltaSummaries.map((summaryLine, idx) => (
                        <li
                          key={idx}
                          className="text-[13px] leading-relaxed text-emerald-900 bg-cc-surface/60 rounded-lg px-3 py-2"
                        >
                          {summaryLine}
                        </li>
                      ))}
      ...
      {showRpBottomSheet && (
        <div className="fixed inset-0 z-[60] flex items-end justify-center">
          ...
          />
          <div className="relative w-full max-w-lg bg-cc-surface rounded-t-2xl shadow-2xl px-5 pt-5 pb-8 animate-in slide-in-from-bottom-4 duration-300">
