...
export default function ClientComplianceScreen() {
  ...
  if (error || !data?.participant) {
    return (
      <ClientScreenShell title="Compliance" backHref={`/client/${id}`}>
  ...
  return (
    <ClientScreenShell title="Compliance" subtitle={data.participant.full_name} backHref={`/client/${id}`}>
      <ScrollView contentContainerStyle={styles.scroll}>
        <Text style={[styles.sectionLabel, { color: colors.mutedForeground, fontFamily: "Inter_700Bold" }]}>
          COMPLIANCE RECORDS
        </Text>
        <ClientSessionList rows={data.compliance} emptyLabel="No compliance records yet" />
      </ScrollView>
    </ClientScreenShell>
  );
}
